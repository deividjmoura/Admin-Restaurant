...
describe('customer session authorization', { skip: !hasDatabase() }, () => {
  ...
  it('log serializers never expose QR or bearer credentials', async () => {
    const memory = createMemoryLogger('info');
    const loggingApp = await buildApp({ loggerInstance: memory.logger });
    try {
      ...
      loggingApp.log.info(
        { authorization: first.headers.authorization, token: first.table.public_token },
        'probe-redact'
      );
      const log = memory.raw();
      assert.ok(log.includes('[redacted]'), 'redação deve aparecer no log');
