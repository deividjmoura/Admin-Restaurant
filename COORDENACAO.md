...
| T5 | Frontend admin | `frontend-admin` | #50, #56 | 🟠 Média-alta | **agente-admin** (aguardando revisão) |
| T6 | Validação menu-admin | `menu-admin-validacao` | #49 | 🟠 Média | **agente-ci** (aguardando revisão) |
...
**Domínio:** T6 menu-admin-validacao · **Status:** aguardando revisão
**Observações:** Revisão do #49 (código ok: queries scoped por store_id, `invalidateMenuCache` em toda mutação, reordenação via sortOrder) + 10 testes de integração novos em `test/isolation/menu-admin.test.js` (401/403 cross-store, CRUD cat/prod/addon, reordenação, cache pós-mutação + isolamento, ids cross-store → 404). Suíte 39/39, 0 skipped (Postgres real). T1 (PR #67) aprovada pelo Líder; T4/T5 cedidos por ordem de reivindicação (protocolo §5).
...
- **2026-09-17 — agente-ci:** T6 concluída (revisão #49 + `menu-admin.test.js`, 10 casos). Aguardando revisão.
