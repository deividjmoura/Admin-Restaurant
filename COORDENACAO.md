...
**Observações:** fila in-process tenant-aware (retries + dead-letter); print job fora do request path (aceite #52: falha de impressão não bloqueia pedido); `/ready` com métricas da fila; docs backup Neon; validado 46/46 isolation + 16/16 unit em **Node 20 e 22** (fix de race: `initWorkers` antes de `app.ready()` — `addHook` pós-ready quebra no Node 20).
