...
import { assertSessionScope } from '../customer/customer-session.js';
import { z } from 'zod';
import {
  getCart,
  addCartItem,
...
async function resolveSessionStore(request, sessionId) {
  assertSessionScope(request.customer, request.storeId, sessionId);
  const { query } = await import('../../infrastructure/db.js');
  const { rows } = await query(
    `SELECT id, store_id, status FROM table_sessions WHERE id = $1 AND store_id = $2`,
    [sessionId, request.storeId]
  );
...
async function cartRoutes(app) {
  ...
  app.get(
    ...
    async (request, reply) => {
      const session = await resolveSessionStore(
        request,
   ...
   */
  app.post(
    '/api/sessions/:sessionId/cart/items',
   ...
   */
  app.patch(
    '/api/sessions/:sessionId/cart/items/:itemId',
    { preHandler: [app.requireCustomerOrPermission('orders.create')] },
    async (request, reply) => {
   ...
   */
  app.post(
    '/api/sessions/:sessionId/cart/checkout',
    { preHandler: [app.requireCustomerOrPermission('orders.create')] },
    async (request, reply) => {
      ...
      if (!session) {
        return send(
          reply,
          new AppError('SESSION_NOT_FOUND', 'Sessão não encontrada.', 404)
        );
      }
