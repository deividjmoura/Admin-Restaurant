...
async function kitchenRoutes(app) {
  const kitchenAccess = {
    preHandler: [
      ...
      app.requireRole(...ROLE_MATRIX['kitchen.board']),
    ],
  };
   ...
   */
  app.get('/api/kitchen/orders', kitchenAccess, async (request, reply) => {
   ...
   */
  app.get('/api/kitchen/events', kitchenAccess, async (request, reply) => {
    const station = parseStation(request.query?.station);
    if (!station) {
      const err = new AppError(
        'INVALID_STATION',
